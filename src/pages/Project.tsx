              ...
              <div className="absolute bottom-0 p-15 right-0 left-0 flex justify-between items-end z-10 text-white  bg-gradient-to-t  from-base-200 to-transparent">
                <div className="w-full lg:w-1/3 space-y-4 ">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {projetActif.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="p-2 py-1 rounded-full backdrop-blur-md border border-base-content/30 bg-base-200/30"
                      >
                        {tech}
                      </span>
                    ))}
                ...
                <div className="flex-col items-end flex">
                  <div className="mb-7 flex">
                    <Link to={`${projetActif.slug}`}>
                      <div className="backdrop-blur-md py-3  px-4 border border-base-content/30 hover:-primary/79 hover:border-primary/80 btn-circle mr-2 ">
                        Voir le projet
                      </div>
                    </Link>
                    <button
                    ...
                    {projetActif.imagesIllustration.map((_, index) => (
                      <div
                        ...
                        className={`bg-base-300 w-7 h-2  rounded-full ${
                          index === imageIndex
                            ? "bg-primary  "
                            : "bg-base-300/50 backdrop-blur-md   shadow"
                        }`}
                      />
                    ))}
                  </div>
